...
def run_rollout(
    ...
    try:
        ## added for ACT, buffer using stack
        from collections import deque
        ...
        for step_i in range(horizon):
            ...
            if using_act:  # set to true if ACT is enabled
                if temperal_agg:
                    ac = policy(ob=ob_dict, goal=goal_dict, t=step_i)
                else:
                    if len(action_buffer) == 0:
                        print("Calculating action chunk...")
                        ac = policy(ob=ob_dict, goal=goal_dict)
                        print(ac.shape)
                        for a in ac[::-1]:
                            action_buffer.append(a)
                    ac = action_buffer.pop()
            else:
                ac = policy(ob=ob_dict, goal=goal_dict)
            ## print(ac)
            ...
            if render:
                env.render(mode="human")
                if (len(cameras)) > 0:
                    time.sleep(0.01)  # wait the rendering to catch up by the camera monitor
