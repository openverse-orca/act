...
def run_example(orcagym_addr : str, 
    ...
    try:
        ...
        if run_mode == "playback":
            ...
            ### override policy if you want to use ACT ###
            if using_act:
                import sys
                sys.path.append(f'{YourPATH}/act')
                from other_envs.franka.policy import PolicyForFranka
                print("Loading ACT policy...")
                policy = PolicyForFranka(
                    f"{YourPATH}/checkpoints", 
                    temperal_agg=temperal_agg,
            ...
            for i in range(rollout_times):
                ...
                print("stats: ", stats)
                total_success_times += stats["Success_Rate"]
            print(f"total_success_times: {total_success_times / rollout_times}; total_rollout_times: {rollout_times}")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--rollout_times', type=int, default=20, help='The times to rollout the policy')
    ...
    parser.add_argument('--use_act', type=bool, default=False, help='The flag to enable the ACT')
    parser.add_argument('--temperal_agg', type=bool, default=False, help='The flag to enable the temperal aggregation')
    ...
    using_act = args.use_act
    temperal_agg = args.temperal_agg
    if args.save_rgb:
        RGB_SIZE = (128, 128)
